export default {
    ...
    subscriptions: {
      ...
      currentPlan: '当前套餐',
      targetPlan: '目标套餐',
      selectTargetPlan: '请选择',
      expirationUnchanged: '到期日保持不变',
      switchCarryRule: '开始时间、额度窗口、日/周/月用量和本月临时额度会原样承接；仅迁移绑定当前订阅分组的 API Key。',
      noSwitchTargets: '没有可切换的同平台订阅套餐',
      switchQuotaDaily: '日 ${value}',
      switchQuotaWeekly: '周 ${value}',
      switchQuotaMonthly: '月 ${value}',
      switchQuotaWarning: {
        title: '切换后以下周期会立即进入余额回退',
        daily: '日用量已达到目标套餐日额度',
        weekly: '周用量已达到目标套餐周额度',
        monthly: '月用量已达到目标套餐月额度（含当前临时额度）'
      },
      resetQuota: '重置配额',
      resetQuotaTitle: '重置用量配额',
      resetQuotaFor: "选择要为 '{user}' 重置的额度周期。",
      resetDailyDesc: '只清零日用量，日窗口从现在重新开始',
      resetWeeklyDesc: '只清零周用量，周窗口从现在重新开始',
      resetMonthlyDesc: '只清零月用量，月窗口从现在重新开始',
      monthlyResetWarning: '重置月额度会同时清除本月临时额度，不影响日、周用量。',
      selectResetPeriod: '请至少选择一个周期',
      quotaResetSuccess: '配额重置成功',
      failedToResetQuota: '重置配额失败',
      monthlyBonus: '临时额度',
      monthlyBonusTitle: '设置本月临时额度',
      monthlyBonusFor: "设置 '{user}' 当前月窗口的临时额外额度。",
      monthlyBonusAmount: '临时额度总值（USD）',
      monthlyBonusHint: '这是当前月的临时额度总值，不是累计增加量；填 0 可清除。不会改变已有用量、日周额度或订阅期限。',
      monthlyBonusValue: '临时 +${bonus}，有效上限 ${limit}',
      monthlyBonusSaved: '本月临时额度已更新',
      invalidMonthlyBonus: '临时额度必须是大于或等于 0 的有效数字',
      failedToSetMonthlyBonus: '设置本月临时额度失败',
      ...
      guide: {
        ...
        actions: {
          ...
          resetQuota: '重置配额',
          resetQuotaDesc: '独立选择日、周、月用量进行重置',
          revoke: '撤销',
