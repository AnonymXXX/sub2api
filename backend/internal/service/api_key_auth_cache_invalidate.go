...
func (s *APIKeyService) InvalidateAuthCacheByUserID(ctx context.Context, userID int64) {
	...
	if err != nil {
		slog.Warn("list api keys for auth cache invalidation failed", "user_id", userID, "error", err)
		return
	}
...
func (s *APIKeyService) InvalidateAuthCacheByGroupID(ctx context.Context, groupID int64) {
	...
	if err != nil {
		slog.Warn("list api keys for auth cache invalidation failed", "group_id", groupID, "error", err)
		return
	}
	s.deleteAuthCacheByKeys(ctx, keys)
}
