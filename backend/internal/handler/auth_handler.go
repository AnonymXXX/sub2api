...
func (h *AuthHandler) ensureBackendModeAllowsUser(ctx context.Context, user *service.User) error {
	...
	if h == nil || !h.isBackendModeEnabled(ctx) || user.CanAccessAdminPanel() {
		return nil
	}
	return infraerrors.Forbidden("BACKEND_MODE_ADMIN_ONLY", "Backend mode is active. Only admin panel login is allowed.")
}
...
func (h *AuthHandler) RefreshToken(c *gin.Context) {
	...
	if h.settingSvc.IsBackendModeEnabled(c.Request.Context()) && !service.CanAccessAdminPanel(result.UserRole) {
